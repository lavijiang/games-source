...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    auto x_bound = std::minmax_element(v.begin(), v.end(),
                                        [](const Eigen::Vector4f& lhs, const Eigen::Vector4f & rhs){
                                            return lhs.x() < rhs.x();
                                        });

    auto y_bound = std::minmax_element(v.begin(), v.end(),  
                                        [](const Eigen::Vector4f& lhs, const Eigen::Vector4f & rhs){
                                            return lhs.y() < rhs.y();
                                        });

    int min_x = floor(x_bound.first->x());
    int max_x = ceil(x_bound.second->x());
    int min_y = floor(y_bound.first->y());
    int max_y = ceil(y_bound.second->y());
